...
| `GetWhereUsed` | 🔍 **WHERE USED ANALYSIS**: Retrieve where-used references for ABAP objects via ADT usageReferences. Shows all places where a specific object (class, program, include, table, bdef, etc.) is used in the system. By default returns minimal relevant results, use detailed=true for complete analysis. | `object_name` (string): Name of the ABAP object to search usages for, `object_type` (string): Type of object (e.g. class, program, include, function, interface, package, table/TABL, bdef/BDEF, etc.), `detailed` (boolean, optional): If true, returns all references including packages and internal components. Default is false (minimal results). | `@tool GetWhereUsed object_name=RM07ALVI object_type=include`, `@tool GetWhereUsed object_name=ZMY_TABLE object_type=table`, `@tool GetWhereUsed object_name=Z_I_MYENTITY object_type=bdef`, or `@tool GetWhereUsed object_name=CL_BUS_ABSTRACT_MAIN_SCREEN object_type=class detailed=true` |
