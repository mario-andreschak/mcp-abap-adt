...
| `GetWhereUsed` | 🔍 **WHERE USED ANALYSIS**: Retrieve where-used references for ABAP objects via ADT usageReferences. Shows all places where a specific object (class, program, include, etc.) is used in the system. | `object_name` (string): Name of the ABAP object to search usages for, `object_type` (string): Type of object (class, program, include, function, interface), `start_position` (object, optional): Starting position in source code with `row` and `col` properties for specific element search, `end_position` (object, optional): End position with `row` and `col` properties | `@tool GetWhereUsed object_name=RM07ALVI object_type=include` or `@tool GetWhereUsed object_name=CL_ABAP_UNIT_ASSERT object_type=class start_position={"row":1,"col":1}` |
