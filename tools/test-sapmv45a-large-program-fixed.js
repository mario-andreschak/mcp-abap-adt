...
const testScenarios = [
    ...
    {
        name: "2. Get includes list (recursive)",
    ...
    {
        name: "3. Get enhancements (main program only)",
        tool: "GetEnhancements",
        args: {
            object_name: PROGRAM_NAME,
    ...
    {
        name: "4. Get enhancements (limited nested - 5 includes)",
        tool: "GetEnhancements",
        args: {
            object_name: PROGRAM_NAME,
            include_nested: true,
    ...
    {
        name: "5. Get enhancements (medium nested - 20 includes)",
        tool: "GetEnhancements", 
        args: {
            object_name: PROGRAM_NAME,
            include_nested: true,
                ...
                try {
                    ...
                    if (responseLine) {
                        ...
                        if (response.result && response.result.content) {
                            ...
                            let summary = '';
                            if (scenario.tool === 'GetProgram') {
                                const lines = resultText.split('\n').length;
                                summary = `📄 Program has ${lines} lines`;
                            } else if (scenario.tool === 'GetIncludesList') {
                            ...
                            } else if (scenario.tool === 'GetEnhancements') {
...
async function main() {
    ...
    const programTest = results.find(r => r.tool === 'GetProgram');
    ...
    const includesTests = results.filter(r => r.tool === 'GetIncludesList');
    ...
    const enhancementTests = results.filter(r => r.tool === 'GetEnhancements');
